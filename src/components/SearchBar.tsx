...
export const SearchBar = () => {
	...
	const handleSuggestions = async (
		...
		const res = await fetch(
			`${GIPHY_API_BASE_URL}${GIPHY_API_SEARCH_SUGGESTIONS_ENDPOINT}?api_key=${process.env.NEXT_PUBLIC_GIPHY_API_KEY}&q=${e.target.value}&limit=${GIPHY_API_LIMIT}`,
		);
		const data = await res.json();
		setSuggestions(
			data.data.map((suggestion: { name: string }) => suggestion.name),
		);
	};
